...
namespace cuts::gOre
{
  ...
  /**
   * @brief π0 rejection cut.
   * @details No subleading gOre shower above threshold + primary-softmax
   * thresholds on the leading photon and leading proton + Δ mass window on
   * the (photon, proton) 4-vector. Applied after the preselection
   * (gOre_topology + single_proton + flash + fiducial + containment).
   * params: { gOre_primary_softmax_min, proton_primary_softmax_min,
   *           delta_mass_center, delta_mass_halfwidth }
   * Defaults match the analysis selection.
   **/
  template<class T>
    bool pi0_rejection(const T& obj,
                       std::vector<double> params = {0.999100, 0.951300, 1232.0, 59.700000})
    {
      ...
      if (idx_proton == kNoMatch)                                    return false;
      auto const& proton = obj.particles.at(idx_proton);
      if (pvars::primary_softmax<T>(proton) <= params.at(1))         return false;
      // Inline delta_mass_P(obj): √(2·p_γ·(E_p − p_p·cosθ) + M_p²)
      double p_gamma  = pvars::gOre::shower_p(*gOre_p);
      double p_proton = pvars::p(proton);
      utilities::three_vector dir_g = utilities::to_three_vector(gOre_p->start_dir);
      utilities::three_vector dir_p = utilities::to_three_vector(proton.start_dir);
      double cosTh = utilities::dot_product(dir_g, dir_p);
      double dm = std::sqrt(2.0 * p_gamma * (std::sqrt(p_proton*p_proton + PROTON_MASS*PROTON_MASS) - p_proton*cosTh) + PROTON_MASS*PROTON_MASS);
      if (std::abs(dm - params.at(2)) >= params.at(3))               return false;
      return true;
    }
  REGISTER_CUT_SCOPE(RegistrationScope::Reco, pi0_rejection, pi0_rejection);

  /**
   * @brief e/γ separation cut.
   * @details Shape and PID requirements on the leading photon candidate.
   * Applied after pi0_rejection.
   * params: { start_dedx_min, axial_spread_min, directional_spread_max,
   *           photon_softmax_min }
   **/
  template<class T>
    bool egamma_separation(const T& obj,
                           std::vector<double> params = {4.070000, -0.500000, 0.037000, 0.922500})
    {
      ...
      if (gOre_p->axial_spread            <= params.at(1))                return false;
      if (gOre_p->directional_spread      >= params.at(2))                return false;
      if (pvars::photon_softmax<T>(*gOre_p) <= params.at(3))              return false;
      return true;
    }
   ...
   * @details Convenience cut for the final selection stage.
   * params: pi0_rejection + egamma_separation defaults concatenated, in
   *   that order. Caller can override either block by supplying all 8
   *   params; the unused defaults remain.
   **/
  template<class T>
    bool full_1g1p_selection(const T& obj,
                             std::vector<double> params = {0.999100, 0.951300, 1232.0, 59.700000,
                                                            4.070000, -0.500000, 0.037000, 0.922500})
    {
      std::vector<double> pi0_p = {params.at(0), params.at(1), params.at(2), params.at(3)};
      std::vector<double> eg_p  = {params.at(4), params.at(5), params.at(6), params.at(7)};
      return pi0_rejection(obj, pi0_p) && egamma_separation(obj, eg_p);
    }
  REGISTER_CUT_SCOPE(RegistrationScope::Reco, full_1g1p_selection, full_1g1p_selection);
